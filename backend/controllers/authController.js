...
const trackCustomerLogin = (customerId, req, success = true) => {
  ...
  db.query("UPDATE customers SET last_login = NOW() WHERE id = ?", [
    customerId,
  ]);
  ...
  db.query(
    ...
    (err, adminResult) => {
      ...
      if (adminResult.length > 0) {
        ...
        // Tidak ditemukan di tabel admin
        // cek customer
        db.query(
          ...
          (err2, custResult) => {
            ...
            if (custResult.length > 0) {
              ...
              console.debug("DEBUG - Customer not found for email:", email);
              return res
                .status(401)
                .json({ message: "Email atau password salah" });
            }
...
export const googleLogin = async (req, res) => {
  ...
  try {
    ...
    if (!verifyRes.ok) {
      ...
      return res
        .status(400)
        .json({ message: "Invalid ID token", details: text });
    }
    ...
    const allowedClientIds = envAllowed
      .split(",")
      .map((s) => s.trim())
      .filter(Boolean);
          ...
          try {
            ...
            db.query(
              ...
              (colErr, colRes) => {
                if (colErr) {
                  console.error(
                    "Failed to read customers table columns:",
                    colErr
                  );
                  return res
                    .status(500)
                    .json({
                      message: "Failed to create user (schema check error)",
                      error: colErr.message || colErr,
                    });
                }
                ...
                if (existingCols.has("password")) {
                  ...
                  insertVals.push(hashed);
                  placeholders.push("?");
                }
                if (existingCols.has("image_url") && picture) {
                  insertCols.push("image_url");
                  insertVals.push(picture);
                  placeholders.push("?");
                }
                ...
                if (insertCols.length === 0) {
                  ...
                  return res
                    .status(500)
                    .json({
                      message: "Failed create customer: no writable columns",
                    });
                }

                const insertQueryDynamic = `INSERT INTO customers (${insertCols.join(
                  ","
                          ...
                          (dupErr, dupRes) => {
                            if (dupErr)
                              return res
                                ...
                                .json({
                                  message:
                                    "Failed fetch existing user after duplicate",
                                  error: dupErr,
                                });
                            if (dupRes.length > 0) {
                              const user = dupRes[0];
                            ...
                            return res
                              ...
                              .json({
                                message:
                                  "Failed create customer: duplicate entry and could not fetch user",
                              });
                          }
                        );
                        return;
            ...
            return res
              .status(500)
              .json({ message: "Error creating user", error: hashErr.message });
          }
        }
      }
    );
    ...
    return res
      .status(500)
      .json({ message: "Google login error", error: err.message });
  }
};
