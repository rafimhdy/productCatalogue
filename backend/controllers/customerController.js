...
export const verifyEmail = (req, res) => {
    ...
    if (!token) {
        return res.status(400).redirect(`${process.env.FRONTEND_URL}/login?message=Verification token is missing.`);
    }

    const query = "SELECT * FROM customers WHERE verification_token = ? AND verification_token_expires > NOW()";

    db.query(query, [token], (err, results) => {
        if (err) {
            console.error("Database error:", err);
            return res.status(500).redirect(`${process.env.FRONTEND_URL}/login?message=Server error during verification.`);
        }

        if (results.length === 0) {
            return res.status(400).redirect(`${process.env.FRONTEND_URL}/login?message=Invalid or expired verification token.`);
        }

        const customer = results[0];

        const updateQuery = "UPDATE customers SET is_verified = 1, verification_token = NULL, verification_token_expires = NULL WHERE id = ?";

        db.query(updateQuery, [customer.id], (updateErr) => {
            if (updateErr) {
                console.error("Error updating customer:", updateErr);
                return res.status(500).redirect(`${process.env.FRONTEND_URL}/login?message=Failed to verify account.`);
            }
...
export const createCustomer = (req, res) => {
    ...
    db.query(
        ...
        (err, result) => {
            ...
            res.json({ message: "Customer created successfully", id: result.insertId });
        }
    );
};
        ...
        db.query(
            ...
            (err, results) => {
                ...
                if (customer.product_categories_preference) {
                    try {
                        customer.product_categories_preference = JSON.parse(customer.product_categories_preference);
                    } catch (e) {
...
export const updateCustomerProfile = (req, res) => {
    try {
        const customerId = req.user.id;
        const { name, phone, newsletter_preference, product_categories_preference } = req.body;
        ...
        if (product_categories_preference !== undefined) {
            updates.product_categories_preference = "product_categories_preference = ?";
        ...
        const updateQuery = `UPDATE customers SET ${Object.values(updates).join(", ")} WHERE id = ?`;
...
export const changeCustomerPassword = (req, res) => {
    try {
        ...
        db.query("SELECT password FROM customers WHERE id = ?", [customerId], (err, results) => {
            if (err) {
