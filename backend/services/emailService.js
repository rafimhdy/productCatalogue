...
export const sendVerificationEmail = async (to, token) => {
  const baseUrl = process.env.BASE_URL || "http://31.97.109.187:5000";
  const verificationUrl = `${baseUrl}/api/customers/verify-email?token=${token}`;
                ...
                ${items
                  .map(
                    (item) =>
                      `<li>${item.name} (x${
                        item.quantity
                      }) - Rp ${item.price.toLocaleString()}</li>`
                  )
                  .join("")}
            </ul>
    ...
    html: `
            ...
            <ul>
                ${items
                  .map(
                    (item) =>
                      `<li>${item.product_name || item.name} (x${
                        item.quantity
                      }) - Rp ${
...
export const sendAdminRefundEmail = async (
  toList,
  customerName,
  orderId,
  items,
