...
const OrderListAdmin = () => {
    ...
    const updateOrderStatus = async (orderId, newStatus) => {
        ...
        try {
            ...
            setSuccessMessage(`Status pesanan #${orderId} berhasil diubah menjadi ${getStatusText(newStatus)}`);
        } catch (err) {
...
<style jsx>{`
    ...
    .status-filter, .items-per-page-select {
        padding: 10px;
        border: 1px solid #ced4da;
        border-radius: 4px;
        font-size: 14px;
    ...
    .status-filter:focus, .items-per-page-select:focus {
        border-color: #007bff;
        outline: none;
    }
